...
export default function Home(props) {
  ...
  const form = useForm({
    initialValues: { name: '', mail: '', message: ' ' },

    validate: {
      name: (value) => (value.length < 2 ? 'Name must be more than 2 letters' : null),
      mail: (value) => (value.length < 5 ? 'Flied must be more than 5 digits' : null),
      message: (value) => (value.length < 10 ? 'Message must be more than 10 characters' : null)
    }
  })
      ...
      >
        <form onSubmit={form.onSubmit(handleSubmit)}
          ...
          />
            ...
            <Button
            type='submit'
            className='px-7 py-3 h-12 w-auto text-white bg-[#667AE1] hover:shadow-lg text-lg rounded-sm'>
              Send
            </Button>
        </form>
      </Modal>
